    ...
    public abstract class Account {
        ...
        private Money dailyWithdrawalToday;
        ...
        //Constructors
        public Account(String accountNumber, Money initialBalance, List<Customer> owners, ProductTier productTier) {
            this.balance = Objects.requireNonNull(initialBalance, "amount cannot be null");
            this.accountNumber = Objects.requireNonNull(accountNumber, "account number cannot be null");
            this.productTier = Objects.requireNonNull(productTier, "product tier cannot be null");
            if(!isValidTier(productTier)){
                throw new IllegalArgumentException(
                        "Product Tier " + productTier.getDisplayName() + " is not allowed for this account type."
                );
            };
    ...
    //        };
            ...
            this.dailyWithdrawalToday = Money.of(new BigDecimal("0.00"), Currency.getInstance("INR"));
            this.lastWithdrawalDate = null;

            if (initialBalance.isLessThan(productTier.getMinBalance())) {
                throw new IllegalArgumentException(
                        "Initial balance " + initialBalance + " is below minimum " +
                                productTier.getMinBalance() + " for " + productTier.getDisplayName()
                );
            }
        ...
        protected abstract boolean isValidTier(ProductTier productTier);
        ...
        public void debit(Money amount) {
            ...
            try {
                ...
                if (lastWithdrawalDate == null || !lastWithdrawalDate.equals(today)) {
                    dailyWithdrawalToday = Money.of(new BigDecimal("0.00"), Currency.getInstance("INR"));
                    lastWithdrawalDate = today;
                }

                Money todayWithdrawal = dailyWithdrawalToday.add(amount);
                Money dailyLimit = getDailyLimit();

                if (todayWithdrawal.compareTo(dailyLimit) > 0) {
                    throw new DailyLimitExceededException(
                            "Daily withdrawal limit of " + dailyLimit + " exceeded! " +
                                    "You have already withdraw " + todayWithdrawal + " today."
                    );
                }
                ...
                this.dailyWithdrawalToday = todayWithdrawal;
            } finally {
        ...
        public void upgradeTier(ProductTier newproductTier) {
            lock.lock();
            try {
                if(!isValidTier(newproductTier)){
                    throw new IllegalArgumentException(
                            "Product Tier " + newproductTier.getDisplayName() + " is not allowed for this account type."
                    );
                }

                if (this.balance.isLessThan(newproductTier.getMinBalance())) {
                    throw new IllegalArgumentException(
                            "Insufficient balance to upgrade to " + newproductTier.getDisplayName() +
                                    ". Required: " + newproductTier.getMinBalance() +
                                    ", Current: " + this.balance
                    );
                }
                this.productTier = newproductTier;
            } finally {
                lock.unlock();
            }
        }
        ...
        public void close() {
            ...
            try {
                ...
                if (!this.balance.isZero()) {
                    throw new AccountFrozenException("Can't close the account with non-zero balance.");
                }
        ...
        public Money getDailyWithdrawalLimit() {
            lock.lock();
            try {
                return this.dailyWithdrawalToday;
            } finally {
                lock.unlock();
            }
        }
